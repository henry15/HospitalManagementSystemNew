...
namespace HospitalManagementSystemNew.Controllers
{
    public class PatientController : Controller
    {
        ...
        public ActionResult Index(string message)
        {
            ...
            var patient = new Patient();// db.Patients.Single(c => c.ApplicationUserId == user);
            ...
            var model = new CollectionOfAll
            {
                ...
                ActiveAppointments = new List<Appointment>(),  //              db.Appointments.Where(c => c.Status).Where(c => c.PatientId == patient.Id).Where(c => c.AppointmentDate >= date).ToList(),
                PendingAppointments = new List<Appointment>(),  //db.Appointments.Where(c => c.Status == false).Where(c => c.PatientId == patient.Id).Where(c => c.AppointmentDate >= date).ToList(),
                AmbulanceDrivers = new List<AmbulanceDriver>(),  //db.AmbulanceDrivers.ToList(),
                Announcements = new List<Announcement>(),  //db.Announcements.Where(c => c.AnnouncementFor == "Patient").ToList()
            };
            return View(model);
        }
       ...
       // [Authorize(Roles = "Patient")]
        ...
        {
            var patient = new Patient();// db.Patients.Single(c => c.ApplicationUserId == id);
            return View(patient);
        }
